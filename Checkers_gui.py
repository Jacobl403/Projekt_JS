...
class Checkers_gui(object):
    ...
    def res_Button(self):
        white = (233, 252, 211)
        light_green = (69, 252, 3)
        pygame.draw.rect(self.screen, light_green, (500, 150, 80, 80))
    ...
    def end_window(self, msg):
        blue = (0, 0, 255)
        light_green = (100, 202, 113)
        running = True
        end_screen = pygame.display.set_mode((500, 200))
        while running:
            ...
            if event.type == pygame.QUIT:
                ...
                running = False
                pygame.quit()
            end_screen.fill(light_green)
            font = pygame.font.Font(None, 25)
            text = font.render(f"Wygrał  {msg}  nacisnij dowolny przycisk  aby wyjść ", True, blue)
            end_screen.blit(text, (25, 50))
        ...
        while self.running:
            try:
                ...
                for event in pygame.event.get():
                    ...
                    # Wybieramy pionka nastepnie sprawdzamy czy nalezy do klasy pawn
                        ...
                        if event.button == 1:
                            ...
                            if isinstance(self.grid.board[old_y][old_x], pawn) and current_player.Check_pawn(
                                    self.grid.board, old_y, old_x):
                                self.second_mouse_click(old_y, old_x, current_player)
                ...
                if self.Player2.number_of_pawns == 0:
                    self.running = False
                    self.end_window("Player 1")
                if self.Player1.number_of_pawns == 0:
                    self.running = False
                    self.end_window("Player 2")
            except Exception as e:
                continue

    def second_mouse_click(self, old_y, old_x, current_player):
        second_click = True
        count_moves = 0
        while second_click:
            event = pygame.event.wait()
